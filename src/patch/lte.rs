...
use super::{
    build::Outcome,
    format::{
        self, Kind, LtePatch, LtePatchCombo, LtePatchComponent, LteSetEntry, SetKind,
        lte_set_entry_key,
    },
};
use crate::{
...
///
/// The optional fields (`ul_bw_class_mimo`, `bcs`, `unknown1`, `unknown2`) are folded with
/// `.unwrap_or(0)`, so this identity treats **absent == present-zero** — a pure
/// `None`-vs-`Some(0)` difference is invisible to `create` (no `change` emitted) and `apply`
/// rehydrates transplanted combos as `Some(0)` (see `format::lte_combo_from_patch`). This is
/// the documented read/write convention (DESIGN.md "Invariants"; reads `.unwrap_or(0)`,
/// writes `Some(value)`); it's value-preserving (both read as 0) and untouched base combos
/// are cloned verbatim, so the base file's own bit-identity is unaffected. Graded latent /
/// by-convention. (Distinguishing the two would
/// be a deliberate patch-format change, not a bug fix.)
...
fn canon(c: &LteCombo) -> CanonLteCombo {
    let mut components: Vec<(i32, i32, i32)> = c
        .components
        .iter()
        .map(|x| (x.band, x.dl_bw_class_mimo, x.ul_bw_class_mimo.unwrap_or(0)))
        .collect();
    ...
    LtePatchCombo {
        components: c
            ...
            .map(|x| LtePatchComponent {
                ...
                ul_bw_class_mimo: x.ul_bw_class_mimo.unwrap_or(0),
            })
            .collect(),
        bcs: c.bcs.unwrap_or(0),
        unknown1: c.unknown1.unwrap_or(0),
...
pub(crate) fn build_lte_patch(a: &[LteCombo], b: &[LteCombo]) -> LtePatch {
    ...
    for (k, bc) in &ib {
        let (differs, kind) = match ia.get(k) {
            ...
            Some(ac) => (canon_variants(ac) != canon_variants(bc), SetKind::Change),
        };
        if differs {
...
mod tests {
    ...
    #[test]
    fn build_lte_patch_treats_absent_optional_as_equal_to_present_zero() {
        // The optional fields (`bcs`, `unknown1`, `unknown2`) fold with `.unwrap_or(0)` in
        // `canon`, so a pure `None`-vs-`Some(0)` difference between A and B must NOT produce
        // any `change`/`add`/`delete` — the two sides are canonically equal. This pins the
        // LTE-side read convention so a future contributor can't "fix" it by distinguishing
        // the two presences and silently breaking round-tripping of real LTE files.
        let with_some = LteCombo {
            components: vec![LteComponent {
                band: 1,
                dl_bw_class_mimo: 32768,
        ...
        let p = build_lte_patch(&[with_some], &[with_none]);
        assert!(p.delete.is_empty(), "{p:?}");
        assert!(p.set.is_empty(), "{p:?}");

        // Sanity check the symmetric direction too.
        let with_some = LteCombo {
            components: vec![LteComponent {
                band: 1,
                dl_bw_class_mimo: 32768,
        ...
        let p = build_lte_patch(&[with_none], &[with_some]);
        assert!(p.delete.is_empty(), "{p:?}");
        assert!(p.set.is_empty(), "{p:?}");
    }
}
