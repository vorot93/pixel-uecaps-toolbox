//! `patch show` — render a combo patch (KDL) in human-readable form.

use super::format::{self, LtePatch, NrPatch, Patch, SetKind, lte_set_entry_key, set_entry_key};
use crate::report::{combos::fmt_cc_features, lte::cc_detail};
...
pub fn show(input: Option<&Path>, full: bool) -> anyhow::Result<i32> {
    ...
    let patch = format::from_kdl(&text)?;
...
/// `+` add, `~` change.
const fn marker(kind: SetKind) -> char {
    match kind {
        SetKind::Add => '+',
        SetKind::Change => '~',
    }
...
/// `add` / `change`, for the `(<kind>)` summary suffix.
const fn kind_label(kind: SetKind) -> &'static str {
    match kind {
        SetKind::Add => "add",
        SetKind::Change => "change",
    }
}

/// Count `set` entries by kind for the header breakdown.
fn count_kinds(kinds: impl Iterator<Item = SetKind>) -> (usize, usize) {
    let (mut add, mut change) = (0, 0);
    for k in kinds {
        match k {
            SetKind::Add => add += 1,
            SetKind::Change => change += 1,
        }
    }
    (add, change)
}
...
fn set_summary(out: &mut String, kind: SetKind, key: &str) {
    let _ = writeln!(out, "  {} {key}  ({})", marker(kind), kind_label(kind));
}

fn render_nr(p: &NrPatch, full: bool, out: &mut String) {
    let (add, change) = count_kinds(p.set.iter().map(|e| e.kind));
    header_and_deletes(out, "nr", p.version, &p.delete, p.set.len(), add, change);
    for e in &p.set {
        let key = set_entry_key(e).expect("validated nr set entry");
        set_summary(out, e.kind, &key);
        if full {
            for combo in &e.combo {
                for cc in &combo.sub_blocks {
                    let raw = cc.to_sub_block().expect("validated nr component");
...
fn render_lte(p: &LtePatch, full: bool, out: &mut String) {
    ...
    for e in &p.set {
        ...
        if full {
            for combo in &e.combo {
                // Reuse the one patch->proto LTE conversion instead of re-inlining it.
                for c in &format::lte_combo_from_patch(combo).components {
                    let _ = writeln!(out, "       B{:<4} {}", c.band, cc_detail(c));
                }
            }
        }
    }
...
mod tests {
    ...
    fn show_reads_a_file() {
        ...
        let path = dir.join("p.kdl");
        std::fs::write(&path, format::to_kdl(&nr_patch()).unwrap()).unwrap();
