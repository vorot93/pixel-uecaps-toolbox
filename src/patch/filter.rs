...
use crate::report::combos::band_label_for;
...
pub fn filter(
    ...
    let mut patch = format::from_kdl(&text)?;
    ...
    let was_empty = patch_is_empty(&patch);

    filter_patch(&mut patch, mode, &wanted);

    if !was_empty && patch_is_empty(&patch) {
...
fn parse_band_arg(s: &str) -> anyhow::Result<String> {
    ...
    // Real band numbers start at 1 (3GPP TS 38.104 / 36.101); reject `n0`/`B0` so a typo
    // doesn't silently match nothing.
    anyhow::ensure!(
        num != "0",
        "invalid band {s:?}; band numbers must be positive"
    );
    Ok(format!("{kind}{num}"))
}
...
mod tests {
    ...
    use crate::patch::format::{
        Kind, LtePatch, LtePatchCombo, LtePatchComponent, LteSetEntry, NrPatch, PatchCombo,
        PatchSubBlock, SetEntry, SetKind, SubBlockKind, lte_set_entry_key, set_entry_key,
    };
    ...
    fn parse_band_arg_canonicalizes_and_rejects() {
        ...
        // Band numbers start at 1: `n0` / `B0` are rejected, not silently accepted.
        assert!(parse_band_arg("n0").is_err());
        assert!(parse_band_arg("B0").is_err());
    }
    ...
    fn filter_roundtrips_through_files() {
        ...
        let inp = dir.join("in.kdl");
        std::fs::write(&inp, format::to_kdl(&nr_patch()).unwrap()).unwrap();
        let outp = dir.join("out.kdl");
        let code = filter(
            FilterMode::Include,
            &["n77".to_string()],
            Some(&inp),
        ...
        .unwrap();
        let parsed = format::from_kdl(&std::fs::read_to_string(&outp).unwrap()).unwrap();
