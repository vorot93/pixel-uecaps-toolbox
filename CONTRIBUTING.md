...
| --- | --- | --- | --- |
| **Clarity/idiom review pass**: dead DTO→payload ingest path deleted; `report::combos::SubBlock` 21 fields → 6 (11 derived display projections + 4 never-read wire fields dropped, projection moved into `fmt_cc_features`); `NrSourceSubBlock`, `ValidatedCarrier` and the LTE identity types re-shaped as sums/proven pairs; `Direction`/`Sku: Display`/`split_raw_band`/`legend_root`/`catalog_indices` introduced as single sources | this pass | Nothing measurable | _`decompose` min-of-5 **16225 ms** vs. the 18463 ms baseline; `provision` min-of-5 **4999 ms** vs. 5215 ms — both faster (load average 2.02 vs. the baseline capture's 2.50, so read this as "no regression", not a win). All three golden hashes byte-identical; all six report outputs (`check` ×2, `matrix`, `inspect --full`, `inspect` legend, `compare --full --common`) byte-identical against unmodified master; full corpus test green_ |
