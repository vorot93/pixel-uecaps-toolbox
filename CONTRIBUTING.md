...
- **Selector-only-stays-unresolved check (1-based leading byte).** `report::combos::feature_index(ids, len)` — reused by `patch::build::self_verify` and both compiler selector checks to confirm a raw selector-only array still does *not* resolve against a (possibly shrunk) list; **not** the real per-CC resolution path — that's `resolve_all(ids, list)` (also in `report::combos`), which resolves a whole direction's array all-or-nothing across every byte. `feature_index`'s first-byte check is sufficient here only because, by construction, a resolved reference's bytes are all-or-none catalog indices.
