...
- **Band prefix (`n`/`B`).** `report::combos::band_label_for(is_nr, band)` is the source. `band_label(raw_band)` *infers* the kind from `NR_BAND_OFFSET`; every caller that already **knows** the kind (`RawSubBlock::band_label`, provision band-drop labels, `patch filter` LTE matching) calls `band_label_for` with its kind. **Do not** route a known-kind band through the inferring `band_label` — it would mislabel an out-of-range value (a stray LTE band ≥ offset would render as NR). This infer-vs-assert split is the subtle trap here.
...
- **Selector-only-stays-unresolved check (1-based leading byte).** `report::combos::feature_index(ids, len)` — reused by `patch::build::self_verify` and the compiler's generated-file self-verification (`verify_compact_feature_list`, `src/compiler/nr.rs`) to confirm a raw selector-only array still does *not* resolve against a (possibly shrunk) list; **not** the real per-CC resolution path — that's `resolve_all(ids, list)` (also in `report::combos`), which resolves a whole direction's array all-or-nothing across every byte. `feature_index`'s first-byte check is sufficient here only because, by construction, a resolved reference's bytes are all-or-none catalog indices.
